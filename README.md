...
resume-tuning 是一个交互式简历生成 skill。给它一份旧简历，或者只口述几段经历，跟你来回几轮，交付一份**链接能点、默认推荐一页、也支持多页的 PDF**——classic / minimal / modern 三种版式随便切，不挑岗位。

默认会优先把简历压到一页，因为多数投递场景更利于快速扫读；资深经历、学术 CV、作品型岗位或用户明确需要完整展开时，可以超过一页，但会说明页数取舍。
